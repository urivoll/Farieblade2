...
public class CocaroachFroledDance : AbstractSpell
{
    ...
    {
        if (inpData["side"] == parentUnit.ParentCircle.Side &&
            inpData["place"] == parentUnit.ParentCircle.Place &&
            inpData["debuffIndex"] == parentUnit.DebuffList.IndexOf(gameObject))
        {
            for (int i = 0; i < inpData["count"]; i++)
            {
                UnitProperties unit = _characterPlacement.CirclesMap[inpData["side"], inpData[$"placeTarget{i}"]].ChildCharacter;
