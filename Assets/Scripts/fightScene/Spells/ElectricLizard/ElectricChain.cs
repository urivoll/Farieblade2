...
public class ElectricChain : AbstractSpell
{
    ...
    public override IEnumerator HitEffect(Dictionary<string, int> inpData)
    {
        ...
        {
            CreateBullet(_characterPlacement.CirclesMap[inpData[$"sideOnMap{i}"], inpData[$"placeOnMap{i}"]].ChildCharacter, inpData[$"damage{i}"]);
        }
        previous = null;
    ...
    private void CreateBullet(UnitProperties unitTarget, int num)
    {
        ...
        unitTarget.HpCharacter.SpellDamage(num, 0);
