...
public class ExistingPassive : AbstractSpell
{
    ...
    {
        if (victim.ParentCircle.Side == parentUnit.ParentCircle.Side &&
            victim.HpCharacter.InpDamageType == 5)
        {
            count++;
            if (count == 4)
            {
                ...
                //parentUnit.GetComponent<UnitProperties>().HpCharacter.damage += Convert.ToInt32(parentUnit.GetComponent<UnitProperties>().HpCharacter.damage * Value);
                parentUnit.GetComponent<UnitProperties>().HpCharacter.HpDamage("dmg");
