...
public class FatManJump : AbstractSpell
{
    ...
    public override IEnumerator HitEffect(Dictionary<string, int> inpData)
    {
        ...
        {
            UnitProperties tempUnit = _characterPlacement.CirclesMap[inpData["sideOnMap"], inpData[$"placeOnMap{i}"]].ChildCharacter;
            if (inpData[$"catch{i}"] == 1)
            {
                ...
                tempUnit.HpCharacter.SpellDamage(inpData[$"damage{i}"], 3);
            }
            //else tempUnit.HpCharacter.Miss();
        }
