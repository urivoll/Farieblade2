...
/*        StartCoroutine(TimeOutTimer());
    ...
    public void AiButton()
    {
        if (PlayerData.ai == 1)
        {
            ...
            PlayerData.ai = 2;
            if (Turns.turnUnit != null && Turns.turnUnit.ParentCircle.Side == BattleNetwork.sideOnBattle && Turns.aiMay) GetComponent<BattleAI>().AI0();
            aiImage.sprite = aiSprite[1];
        }
    ...
    public void FixedUpdate()
    {
        ...
        float newFillAmount = playersBars[Turns.turnUnit.ParentCircle.Side].fillAmount - (fillSpeed * Time.fixedDeltaTime);
        playersBars[Turns.turnUnit.ParentCircle.Side].fillAmount = Mathf.Clamp01(newFillAmount);

        // »змен€ем цвет в соответствии с заполнением
        float fillPercentage = playersBars[Turns.turnUnit.ParentCircle.Side].fillAmount;
        Color color;
        ...
        playersBars[Turns.turnUnit.ParentCircle.Side].color = color;
