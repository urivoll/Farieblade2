...
public class EdwardPassive : AbstractSpell
{
    ...
    public void Up()
    {
        ...
        {
            count = 0;
            if (parentUnit.HpCharacter.heal == true)
            {
                Instantiate(heal, parentUnit.PathBulletTarget.position, Quaternion.identity);
                //parentUnit.HpCharacter.hp += Convert.ToInt32(parentUnit.HpCharacter.hpBase * Value);
                if (parentUnit.HpCharacter.Hp > fromUnit.hpBase) //parentUnit.HpCharacter.hp = fromUnit.hpBase;
