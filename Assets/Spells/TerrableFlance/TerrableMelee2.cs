...
public class TerrableMelee2 : AbstractSpell
{
    ...
    {
        yield return null;
        //_characterPlacement.CirclesMap[inpData["side"], inpData["place"]].newObject.accuracy = inpData["youAcc"];
        //_characterPlacement.CirclesMap[inpData["sideEnemy"], inpData["placeEnemy"]].newObject.accuracy = inpData["enemyAcc"];
        if (inpData.ContainsKey("heal"))
            //_characterPlacement.CirclesMap[inpData["side"], inpData["place"]].newObject.hp = inpData["heal"];
        if (inpData.ContainsKey("stuck"))
            _characterPlacement.CirclesMap[inpData["sideEnemy"], inpData["placeEnemy"]].ChildCharacter.DebuffList[inpData["stuck"]].GetComponent<AbstractSpell>().StuckMethod();
    }
