    ...
    private void Cast(UnitProperties victim, Dictionary<string, int> inpData)
    {
            ...
            inpData["debuffId"] == id)
        {
            ...
            if (inpData["effect"] == 0)
            {
                ...
                //if (PlayerData.language == 0)
                    //newObj.transform.Find("TextDamage/Text").GetComponent<TextMeshProUGUI>().text = $"+{Convert.ToInt32(unit.damage * 0.2f)} Урона";
                //else
                    //newObj.transform.Find("TextDamage/Text").GetComponent<TextMeshProUGUI>().text = $"+{Convert.ToInt32(unit.damage * 0.2f)} Damage";
            }
            ...
            {
                //unit.HpCharacter.hp += Convert.ToInt32(unit.HpCharacter.hp * 0.2f);
                unit.HpCharacter.HpDamage("hp");
                if (PlayerData.language == 0) newObj.transform.Find("TextDamage/Text").GetComponent<TextMeshProUGUI>().text = $"+{Convert.ToInt32(unit.HpCharacter.Hp * 0.2f)} Здоровья";
                else newObj.transform.Find("TextDamage/Text").GetComponent<TextMeshProUGUI>().text = $"+{Convert.ToInt32(unit.HpCharacter.Hp * 0.2f)} HP";
            }
