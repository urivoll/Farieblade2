...
public class BattleNetwork : MonoBehaviour
{
    ...
    //«¿œ–Œ— Õ¿ ¿“¿ ”   —≈–¬≈–”
    public void AttackQuery(int spell, int mode, string ident, int side = -666, int place = -666)
    {
        try
        {
            Debug.Log("—ÚÓÓÌ‡: " + side + ", ÃÂÒÚÓ: " + place + ", —ÔÓÒÓ·ÌÓÒÚ¸: " + spell);
            hubConnection.SendAsync("AttackHandler", side, place, gameIndex, sideOnBattle, spell, mode, ident);
        }
        catch (Exception ex)
        {
            print(ex.ToString());
