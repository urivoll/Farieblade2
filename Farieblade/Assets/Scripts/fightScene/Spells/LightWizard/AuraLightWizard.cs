...
public class AuraLightWizard : Aura
{
    ...
    public override IEnumerator GetAura(Dictionary<string, int> inpData)
    {
        ...
        {
            Instantiate(effect, _characterPlacement.CirclesMap[inpData["side"], inpData[$"place{i}"]].newObject.pathBulletTarget.position, Quaternion.identity);
            _characterPlacement.CirclesMap[inpData["side"], inpData[$"place{i}"]].newObject.pathEnergy.SetEnergy(inpData[$"energy{i}"]);
        }
        yield return new WaitForSeconds(0.3f);
