...
public class NecromancerResurect : AbstractSpell
{
    ...
    private void CastDebuff(UnitProperties Victim, Dictionary<string, int> inpData)
    {
        if (inpData.ContainsKey("sideFrom") &&
            ...
            inpData["debuffId"] == id)
        {
            UnitProperties unit = _characterPlacement.CirclesMap[inpData["side"], inpData["place"]].newObject;
            unit.resurect = true;
        }
    }
    public override IEnumerator AfterStep(Dictionary<string, int> inpData)
    {
        ...
        UnitProperties unit = _characterPlacement.CirclesMap[inpData["sideTarget"], inpData["placeTarget"]].newObject;
