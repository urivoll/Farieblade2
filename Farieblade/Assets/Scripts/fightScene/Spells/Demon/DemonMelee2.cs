    ...
    public override IEnumerator HitEffect(Dictionary<string, int> inpData)
    {
        ...
        for (int i = 0; i < _characterPlacement.UnitOur.Count; i++)
        {
            ...
            if (_characterPlacement.UnitOur[i] != fromUnit.Model)
            {
                ...
                _characterPlacement.UnitOur[i].SpellDamage(1, 5);
        ...
        _characterPlacement.CirclesMap[inpData["sideOnMap"], inpData["placeOnMap"]].newObject.SpellDamage(inpData["damage"], 4);
        if(inpData["count"] == 2)
            _characterPlacement.CirclesMap[inpData["sideOnMap"], inpData["placeOnMapBehind"]].newObject.SpellDamage(inpData["damageBehind"], 4);
