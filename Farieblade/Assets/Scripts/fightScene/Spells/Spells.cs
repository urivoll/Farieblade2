...
public class Spells : MonoBehaviour
{
    ...
    public IEnumerator UseActiveAsync(int i, List<MakeMove> inpData)
    {
        ...
        if (currentSpell.state == "Effect")
        {
            ...
            GameObject newObject = Instantiate(SpellList[i], _characterPlacement.CirclesMap[inpData[0].attackSend["side"], inpData[0].attackSend["place"]].newObject.pathDebuffs);
        ...
        else if (currentSpell.state == "Ball")
        {
            ...
            while (count < currentSpell.times)
            {
                ...
                if (_characterPlacement.CirclesMap[inpData[count].attackSend["side"], inpData[count].attackSend["place"]].newObject != null)
                    bulletTarget = _characterPlacement.CirclesMap[inpData[count].attackSend["side"], inpData[count].attackSend["place"]].newObject.pathBulletTarget.gameObject;
                ...
                bullet.unitTarget = _characterPlacement.CirclesMap[inpData[count].attackSend["side"], inpData[count].attackSend["place"]].newObject;
        ...
        else if (currentSpell.state == "Melee")
        {
            ...
            while (count != currentSpell.times)
            {
                ...
                if (inpData[count].attackSend.ContainsKey("damage"))
                    _characterPlacement.CirclesMap[inpData[count].attackSend["side"], inpData[count].attackSend["place"]].newObject.SpellDamage(inpData[count].attackSend["damage"], inpData[count].attackSend["element"]);
                count++;
                yield return new WaitForSeconds(currentSpell.between);
            }
            PlayDoneSound(currentSpell);
