...
public class DeadKingResurect : AbstractSpell
{
    ...
    public override IEnumerator AfterStep(Dictionary<string, int> inpData)
    {
        ...
        {
            list.Add(_characterPlacement.CirclesMap[inpData["side"], inpData[$"placeOur{i}"]].newObject);
            GameObject newObject = Instantiate(soul, _characterPlacement.CirclesMap[inpData["side"], inpData["victim"]].newObject.transform.position, Quaternion.identity);
            newObject.transform.Find("DeadKingSoulTarget").gameObject.transform.position = _characterPlacement.CirclesMap[inpData["side"], inpData[$"placeOur{i}"]].newObject.pathBulletTarget.position;
        }
